...
三层取值后，用户自定义原样返回；选中词典层释义且全部内容仅描述词形关系时，在读取期补充本地原形的有效语义。`meaningSource` 仍标记选中的原始层，原词的 headword、音标、词形关系和数据库中的释义字段均不改写。列表、详情与开始复习共用这一规则，已有生词立即适用，无需重新导入。

- 读取时统一解码旧数据中的字面量 `\n`，与实际换行、分号一起拆分；动态补充的实际语义也统一输出真实换行，不依赖前端按 `meaningSource` 解码，无需重新导入。识别过去式、过去分词、现在分词、第三人称单数、复数、比较级与最高级等明确关系；含独立词义、无法解析或指向多个原形时不补全。
- 原形优先取结构化 `exchange["0"]`、非自身的 `lemma`，缺失时才取关系文本中的单一原形；结构化信息与文本冲突、目标为自身或不是单一词时安全回退。
- 一次批量读取所有目标原形，使用原形的 `review_meanings ?? raw_meanings`；逐片段过滤空内容与词形关系说明，只追加实际语义（混合义项也不携带原形的其他关系），保留原关系说明及原形义项的词性。原形本身仍只有关系说明（包括循环引用）时不继续追溯，回退到原释义。
- 目标不存在、无释义或无法确定时不影响复习；数据库读取故障仍按正常错误链路报告。
- 这是展示补全，不改变导入时的 [词形归一](normalization.md) 与学习数据归属；搜索仍匹配已存储的单词及释义，不扩展为原形释义搜索。
