...
- 识别过去式、过去分词、现在分词、第三人称单数、复数、比较级与最高级等明确关系；含独立词义、无法解析或指向多个原形时不补全。
- 原形优先取结构化 `exchange["0"]`、非自身的 `lemma`，缺失时才取关系文本中的单一原形；结构化信息与文本冲突、目标为自身或不是单一词时安全回退。
- 一次批量读取所有目标原形，使用原形的 `review_meanings ?? raw_meanings`；只追加非空且包含实际语义的内容，保留原关系说明及原形义项的词性。原形本身仍只有关系说明（包括循环引用）时不继续追溯，回退到原释义。
