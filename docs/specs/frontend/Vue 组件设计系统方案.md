...
本方案用于 Vue 组件系统，以 HeroUI v3 为视觉参考；项目组件选型仍以《[前端技术栈](前端技术栈.md)》为准，不引入 React 组件实现。
...
以上映射不等于组件已经通过可访问性验收。Soft 背景、正文、边界、焦点以及所有交互状态仍需在真实组合中验证，要求见《[前端交互与可访问性规范](前端交互与可访问性规范.md)》。
...
HeroUI 参考样式的按钮按屏幕宽度调整高度：桌面常见 `sm / md / lg` 为 32 / 36 / 40px，较窄屏幕为 36 / 40 / 44px。**本项目不复制按断点缩小触摸目标的规则**，默认交互目标遵循《[前端交互与可访问性规范](前端交互与可访问性规范.md)》的 44px 基线。
...
动效采用《[前端交互与可访问性规范](前端交互与可访问性规范.md)》的时间基线：
...
具体键盘、表单、弹层、对比度和测试门槛分别见《[前端交互与可访问性规范](前端交互与可访问性规范.md)》与《[前端测试规范](前端测试规范.md)》。
