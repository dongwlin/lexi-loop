# Reka UI + Tailwind CSS v4 + Radix Colors 设计系统方案
...
本方案用于 Vue 组件系统，与 HeroUI v3 的视觉语言保持接近。

“向 HeroUI 对齐”包括颜色关系、表面层级、圆角、尺寸、字体、组件变体和交互反馈，不只是把主按钮改成蓝色。它不意味着复制 HeroUI 的 DOM、状态属性或全部主题变量。
...
HeroUI v3 参考核对日期为 **2026-09-03**。其默认主题和组件样式是视觉参考，不是本方案的运行时依赖；后续落地应锁定实际参考版本，避免跟随上游分支静默改变外观。
...
避免把“圆润”扩展成所有区域都套圆角卡片，也不要把官网中的渐变大字、光晕和玻璃背景作为业务界面默认样式。表格、长列表和普通正文仍以信息密度与阅读顺序为先。
...
HeroUI 明确区分常驻表面、弹层表面和遮罩；本方案采用同样的角色区分。
...
不机械限制为“10～20 个 Token”，而是只保留已经存在的视觉角色：
...
- 暂不为每个 Button、Badge、Menu Item 建一套独立颜色变量。
- 页面通过组件 Variant 选择外观，不自行拼接一套新的状态规则。
...
Radix 的 12 个 Step 提供有用的起点：1–2 为背景、3–5 为交互填充、6–8 为边界、9–10 为实心颜色、11–12 为文字。它们是用途建议，不要求所有主题都把 Card 固定映射到 Step 2，也不保证任意前景与背景组合满足项目对比度要求。[Radix 色阶用途](https://www.radix-ui.com/colors/docs/palette-composition/understanding-the-scale)
...
这是一套 **HeroUI 风格的 Radix 适配主题**，不是 HeroUI 默认主题的逐像素复刻。Gray 与 HeroUI 的中性色并不完全相同，Blue 也不等于 HeroUI 的 Accent。

### 4.2 品牌蓝必须校准

HeroUI v3 参考主色为 `oklch(0.6204 0.195 253.83)`。本方案保留其蓝色色相方向，但为常用小字号白字按钮降低亮度，采用以下项目值。
...
以上映射不等于组件已经通过可访问性验收。Soft 背景、正文、边界、焦点以及所有交互状态仍需在真实组合中验证，具体要求见《[[前端交互与可访问性规范]]》。
...
主题变量和 CSS-first 配置放在下面的 `theme.css` 中。

### 5.2 Primitive 与 Semantic Tokens

Radix 的 CSS 文件会为 `:root / .light` 和 `.dark` 提供对应色阶，因此需要同时导入浅深两套。应用使用根元素上的 `.light` / `.dark` 切换，不能只设置 Radix CSS 不识别的 `data-theme`。[Radix CSS 用法](https://www.radix-ui.com/colors/docs/overview/usage)
...
共同的语义别名在每个主题作用域重新声明，避免局部主题继承到已经在父级解析好的颜色。当前默认仍是根级统一主题；局部主题与 Portal 的组合需要额外处理继承边界。

### 5.3 暴露给 Tailwind

引用其他 CSS 变量时使用 `@theme inline`，使 Utility 在使用位置读取当前主题语义变量。[Tailwind Theme Variables](https://tailwindcss.com/docs/theme)
...
使用 `rounded-control / rounded-field / rounded-card` 等角色名，避免直接覆盖 Tailwind 的整套 `rounded-sm / md / lg`，导致现有布局中的圆角含义一起变化。颜色、圆角和阴影都集中在主题层；Spacing 继续使用 Tailwind 原生刻度。
...
HeroUI v3 参考源码中，默认基础圆角为 8px，Button 使用其 24px 圆角级别，Input 使用 12px 的 Field 圆角，Card 默认对应 24px。本方案将这些形态转成稳定的组件角色 Token，而不是照搬类名。
...
Popover 与 Menu Item 的数值是本方案的配套选择，不声称与所有 HeroUI 组件一一相同。不要把 Textarea、表格容器和大型面板统一做成胶囊形。

### 6.2 尺寸与密度

HeroUI 参考样式的按钮会按屏幕宽度调整高度：桌面常见 `sm / md / lg` 为 32 / 36 / 40px，较窄屏幕为 36 / 40 / 44px。**本项目不直接复制按断点缩小触摸目标的规则**，默认交互目标遵循《[[前端交互与可访问性规范]]》的 44px 基线。
...
使用最小高度而不是固定高度承载可能换行的内容。长中文、错误提示和字体放大后，应增加高度而不是截断。

### 6.3 排版

- 默认使用系统无衬线字体栈并补齐中文回退，不为接近 HeroUI 额外引入装饰字体。
- 正文和输入文字以 14–16px 为主；移动输入框优先 16px。
- 按钮、Label 和常规卡片标题使用 Medium；分区标题可用 Semibold。
- 卡片标题不必统一放大到 20–24px，重要性通过层级和间距体现。
- 次要文字使用 `text-muted-foreground`，不要对整段内容设置低透明度。
- 金额、时间、计数等需要纵向对齐时使用 `tabular-nums`。
...
采用“浅灰画布 → 白色表面 → 柔和阴影”的关系。普通 Card 默认 `bg-surface shadow-surface`，不再统一附加 `border border-border`。

阴影只负责轻量分离，不使用品牌色发光阴影，也不让普通信息卡片在 Hover 时全部上浮。只有可点击的卡片才提供交互反馈。

### 7.2 深色主题

深色主题主要依靠画布、卡片和弹层的明度差。Card 与 Field 可以取消外投影；Overlay 保留轻微内描边，避免浮层与下方内容融为一体。

`surface-secondary`、`surface-tertiary` 表达嵌套背景，并非“颜色越亮 z-index 越高”。遮挡顺序仍由组件定位与统一层级策略控制。
...
| Outline Button | 显式边框，不能与 Filled 混成同一种变体 |
| 必须靠轮廓识别的输入框 | `border-field-border`，确保实际边界清晰 |
| Focus | 独立的 `ring` 色，不用普通装饰边框代替 |
| Invalid | 错误文字 + 错误指示，不能只有红色轮廓 |

HeroUI 默认 Input 使用白色 Field、轻阴影和无可见边框，也提供中性填充的 Secondary 变体。本方案允许这两种外观，但不能以”官方默认无边框”为由豁免控件可辨识性检查。白色卡片上的白色输入框尤其要检查，必要时使用显式描边变体。
...
业务 API 使用语义 Variant。不要沿用 HeroUI v2 的 `flat / light / bordered / shadow` 分类，再与 v3 名称混用。
...
这里的 `secondary` 是中性底上的品牌文字，不机械等同于浅蓝色实心块；它与 `tertiary` 的区分参考当前 HeroUI v3 Button 样式。`danger-soft` 用于低强调危险入口，最终不可逆确认可升级为 `danger`。
...
<button
  ...
  class="
    ...
    outline-hidden focus-visible:outline-2
...
这是样式示例，实际由共享 Button 组件封装。Disabled 使用原生 `disabled`；Pending 保留按钮宽度与可读文案、表达 `aria-busy` 并阻止重复激活。仅设置 `aria-disabled` 或 `pointer-events-none` 不会自动阻止键盘触发。

### 8.2 Field

Label 放在控件外部并保持可见；Placeholder 只提供示例。默认 Field 与 Secondary Filled Field 共享圆角、尺寸、焦点和错误规范，不让每个表单自行选择风格。
...
Invalid 状态设置真实的 `aria-invalid`、关联错误信息，并保留可见焦点；不要让错误色覆盖掉键盘 Focus。Secondary Filled 变体可采用 `bg-default` 与无阴影，但是否保留边框仍由实际背景中的可辨识性决定。

### 8.3 其他组件

| 组件 | 视觉配方与注意点 |
| --- | --- |
| Card | `rounded-card bg-surface p-4 shadow-surface`；不默认可点击 |
| Dialog | `rounded-dialog bg-overlay p-6 shadow-overlay`；Backdrop 独立；标题、说明、关闭与底部操作位置一致 |
| Dropdown / Popover | `rounded-popover bg-overlay p-1 shadow-overlay`；内容保持不透明，避免文字叠影 |
| Menu Item | `rounded-item`；高亮用 `default-hover`，与深色 Overlay 区分；危险项用 `danger-text` 与 `danger-subtle` |
...
普通按钮、输入框和卡片可使用原生 HTML 封装，不要求为每一种基础元素寻找 Reka Primitive。Dialog、Menu、Select、Tabs 等复杂交互则交给 Reka。

---

## 9. Reka UI 状态接入

样式读取 Reka 实际暴露的状态，不自行维护一份 Hover / Selected 状态，也不把其他组件库的属性名称复制过来。
...
下面是一个只负责“发出重命名意图”的小型 SFC，完整保留 Trigger、Portal、Content 与 Item 结构：
...
菜单动作通过 Reka 的 Select 事件处理，避免仅监听鼠标点击。生产封装仍需透传必要的 Props、Attrs 与事件；不要覆盖 Reka 的键盘、焦点恢复或关闭逻辑。Dialog 需要其 Title、Description 等语义组成，不能用一张外观相似的 Card 替代。
...
- 不为视觉对齐默认引入 Framer Motion、涟漪依赖或持续 `will-change`。
...
不在每个组件重复写 `bg-white dark:bg-...`。阴影也通过主题变量切换，不需要四处增加 `dark:shadow-none`。`dark:` 只保留给图片亮度、主题专属资产等确实不同的视觉行为。
...
- 复杂组件按实际需要拆文件；先不建设独立 Design Token 包或跨框架组件仓库。
- Variant 类名保持完整、可静态发现，不拼接 `bg-${color}-500` 一类字符串。

建议按以下顺序落地到实际应用：

1. 先校准画布、表面、文字与主按钮，检查浅深两套主题。
2. 完成 Button、Field、Card 三个基础配方。
3. 接入 Menu、Select、Dialog，检查 Portal、Focus 与状态样式。
4. 补充 Chip、Tabs、选择控件，再放进真实表单与列表页面验证。
5. 最后收敛间距、字体和动效；不要只在孤立色块上验收设计系统。

---

## 13. 视觉验收与迁移检查

在实际应用的组件工作台中，用相同文案、相近尺寸并排对照已锁定版本的 HeroUI v3 默认示例。验收目标是视觉语言一致。

- [ ] 浅色页面有清楚的灰底 / 白色表面关系，不是白底上堆灰框。
- [ ] 深色 Card、嵌套区域和 Overlay 能区分，不只删除所有阴影后结束。
...
- [ ] 键盘焦点可见；Disabled、Pending、Invalid 不会误触发操作。
- [ ] `overlay` 与 `backdrop` 已按真实用途迁移。
- [ ] 若旧组件用到 `primary-border`，已按用途改为 `primary-text` 或独立的语义边界，而非保留未定义 Token。
- [ ] 所有示例依赖的 Token 都有浅深主题值，组件未残留 Slate / Indigo 的直接颜色绑定。
- [ ] 默认触摸目标、长中文、文字缩放和窄视口满足项目规范。
- [ ] 已在实际前景 / 背景组合下验证对比度，不以 Radix 或 HeroUI 的名称代替检查。
...
> [!success] 最终方向
>
> **保留 Reka UI + Tailwind CSS v4 + Radix Colors 的实现基础，用中性画布、蓝色强调、圆润控件、明确表面层级与轻量交互，建立向 HeroUI v3 对齐的 Vue 设计系统。**
