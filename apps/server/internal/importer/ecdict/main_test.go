...
// 跑完统一清理。导入落点为真实 schema，因此先执行 migrations.Migrate。
func TestMain(m *testing.M) {
	...
	container, err := tcpostgres.Run(ctx, pgImage,
		...
		testcontainers.WithWaitStrategy(
			...
			// 等就绪日志出现两次再开始（与 migrations / repo 包相同）。
...
// truncateDictionary 清空全部业务表。集成用例共享一个数据库，表级用例
// 不并行，各用例开始时清空以保证互相独立（Go 测试规范：共享数据库状态
// 不做并行）。
func truncateDictionary(t *testing.T) {
	t.Helper()
...
// countDictionaryEntries 统计 dictionary_entries 当前行数。
func countDictionaryEntries(t *testing.T) int {
	t.Helper()
	n, err := testDB.NewSelect().TableExpr("dictionary_entries").Count(context.Background())
	require.NoError(t, err, "统计词条行数失败")
	return n
}
