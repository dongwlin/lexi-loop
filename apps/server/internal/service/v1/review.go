...
func (s *Review) StartSession(ctx context.Context, req service.StartSessionRequest) (*service.StartSessionResult, error) {
	if req.Count < 1 {
		return nil, apperr.New(apperr.InvalidArgument, apperr.CodeValidationFailed, "count must be positive", nil)
	}

	var result *service.StartSessionResult
...
// items 的时间戳全部一致。
func (s *Review) startSessionTx(ctx context.Context, tx bun.Tx, count int, now time.Time) (*service.StartSessionResult, error) {
	...
	result := &service.StartSessionResult{
		SessionID:      session.ID,
		RequestedCount: count,
		TotalCount:     total,
		Items:          make([]service.StartSessionItem, 0, len(items)),
	}
	for i, item := range items {
		...
		result.Items = append(result.Items, service.StartSessionItem{
			ItemID:           item.ID,
...
func (s *Review) SubmitResult(ctx context.Context, req service.SubmitResultRequest) error {
...
func (s *Review) submitResultTx(ctx context.Context, tx bun.Tx, req service.SubmitResultRequest, now time.Time) error {
	reviewRepo := repo.NewReviewRepo(tx)
	userWordRepo := repo.NewUserWordRepo(tx)
...
func (s *Review) AbandonSession(ctx context.Context, req service.AbandonSessionRequest) error {
	err := s.db.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		reviewRepo := repo.NewReviewRepo(tx)
		session, err := reviewRepo.LockSessionByID(ctx, req.SessionID)
		if err != nil {
...
func (s *Review) GetSession(ctx context.Context, req service.GetSessionRequest) (*service.GetSessionResult, error) {
	...
	result := &service.GetSessionResult{
		SessionID:   session.ID,
		...
		Items:       make([]service.GetSessionItem, 0, len(items)),
	}
	for _, row := range items {
		result.Items = append(result.Items, service.GetSessionItem{Headword: row.Headword, Result: row.Item.Result})
	}
	return result, nil
}
...
var _ service.Review = (*Review)(nil)
