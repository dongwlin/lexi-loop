...
// 导入编排依赖 ecdict.Importer（structure.md §1 的 service/v1 → importer
...
type DictImport struct {
	...
	snap    service.DictImportSnapshot
}
...
func NewDictImport(db *bun.DB, importer *ecdict.Importer, csvPath string, autoCheck bool, log zerolog.Logger) *DictImport {
	state := service.DictImportStateIdle
	if autoCheck {
		state = service.DictImportStateChecking
	}
	return &DictImport{
		db:           db,
		importer:     importer,
		...
		autoCheck:    autoCheck,
		log:          log,
		snap:         service.DictImportSnapshot{State: state},
	}
}
...
func (s *DictImport) Snapshot() service.DictImportSnapshot {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.snap
}
...
func (s *DictImport) skip() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.snap = service.DictImportSnapshot{State: service.DictImportStateIdle}
}
...
func (s *DictImport) beginChecking(m *dictManifest) {
	...
	defer s.mu.Unlock()
	now := time.Now().UTC()
	s.snap = service.DictImportSnapshot{
		State:         service.DictImportStateChecking,
...
func (s *DictImport) beginImport() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.snap.State = service.DictImportStateImporting
...
func (s *DictImport) updateProgress(rowsProcessed, entriesWritten int64) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.snap.State != service.DictImportStateImporting {
		return
	}
...
func (s *DictImport) complete(rowsProcessed, entriesWritten int64) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.snap.State = service.DictImportStateCompleted
	s.snap.RowsProcessed = rowsProcessed
	s.snap.EntriesWritten = entriesWritten
	s.snap.UpdatedAt = time.Now().UTC()
}
...
func (s *DictImport) fail(err error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.snap.State = service.DictImportStateFailed
	s.snap.ErrorMessage = err.Error()
	s.snap.UpdatedAt = time.Now().UTC()
}

var _ service.DictImport = (*DictImport)(nil)
