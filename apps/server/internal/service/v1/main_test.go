...
// Service 层测试使用真实 Repo + 真实数据库，不 mock（架构规范 §11.3）。
...
func TestMain(m *testing.M) {
	...
	container, err := tcpostgres.Run(ctx, pgImage,
		...
		testcontainers.WithWaitStrategy(
			...
			// 等就绪日志出现两次再开始（与 repo / migrations / infra/database 相同）。
...
// resetTables 清空全部业务表。集成用例共享一个数据库且涉及全局不变量
// （如全库至多一个 active session），表级用例不并行，各用例开始时清空
// 以保证互相独立（Go 测试规范：共享数据库状态不做并行）。
