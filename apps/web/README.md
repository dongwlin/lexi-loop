...
- [ ] 纯逻辑单元测试：复习状态机迁移、session 快照存取、键盘映射（导入解析聚合已随导入页落地，生词库的分页窗口 / query 参数解析 / 释义格式化已随生词库落地；组件 / 页面集成测试待下方质量设施落地后补）
